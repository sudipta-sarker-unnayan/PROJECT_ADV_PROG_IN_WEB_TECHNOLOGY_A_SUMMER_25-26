...
export class TaskController {
  ...
  async getMyTasksAsClient(@CurrentUser() user: CurrentUserPayload): Promise<Task[]> {
    return this.taskService.getMyTasksAsClient(user.userId);
  }

  @Get(':id')
  @Roles(RoleName.SUPER_ADMIN, RoleName.MANAGER, RoleName.EMPLOYEE, RoleName.CLIENT)
  ...
  @Roles(RoleName.SUPER_ADMIN, RoleName.MANAGER)
  async getTaskByEmployee(@Param('employeeId')employeeId: number):Promise<Task[]>{
  ...
  async uploadCompletedFile(@Param('id', ParseIntPipe) id: number,@UploadedFile() file: UploadedFileInfo | undefined,@CurrentUser() user: CurrentUserPayload):Promise<Task>{
  ...
  async updateProgress(@Param('id')id: number,@Body() updateProgressDto:UpdateProgressDto):Promise<Task>{
