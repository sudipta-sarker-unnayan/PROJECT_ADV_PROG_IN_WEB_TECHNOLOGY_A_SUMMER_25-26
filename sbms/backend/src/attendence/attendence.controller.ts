...
export class AttendenceController {
    ...
    async getMyAttendence(@CurrentUser() user: CurrentUserPayload): Promise<Attendence[]> {
        return this.attendenceSrevice.getMyAttendence(user.userId);
    }

    @Post()
    @Roles(RoleName.SUPER_ADMIN,RoleName.MANAGER)
    async giveAttendence(@Body()createAttendenceDto:CreateAttendenceDto):Promise<Attendence>{
        const attendence = await this.attendenceSrevice.giveAttendence(createAttendenceDto)
        return attendence
    }
